...
DistanceBetweenOneClusterAndOneTrack::DistanceBetweenOneClusterAndOneTrack() : Distance(), trackParams(0), normU(0)
{
  /*
...
{
  ThreeVector C(cluster->getClusterPosition().X(),cluster->getClusterPosition().X(),cluster->getClusterPosition().Z());
  ThreeVector BC(B.X()-C.X(),B.Y()-C.Y(),B.Z()-C.Z());
  if(normU>0)
    return (BC.Cross(u)).Mag()*(1.f/normU);
  else{
...
DistanceBetweenOneHitAndOneTrack::DistanceBetweenOneHitAndOneTrack() : Distance(), trackParams(0), normU(0)
{  
  /*
    hit H(x_c,y_c,z_c)
    track T : x_t = param[0] + param[1]*z_t => plan equation; normal vector Nx(-1,0,param[1])
